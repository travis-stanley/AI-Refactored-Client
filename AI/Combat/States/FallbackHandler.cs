// <auto-generated>
//   AI-Refactored: FallbackHandler.cs (May 2025 Realism – BotBrain-Controlled, FSM-Contract Edition)
//   All fallback logic is fully controlled by BotBrain. All public APIs required by FSM are present.
//   Exposes retry, anticipation, arrival, path/trigger checks for full CombatStateMachine/echo integration.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Combat.States
{
    ...
    public sealed class FallbackHandler : IDisposable
    {
        ...
        // --- FSM-required APIs below ---

        /// <summary>
        /// FSM: Should fallback handler be in use now? (classic "engaged" test)
        /// </summary>
        public bool ShallUseNow(float now)
        {
            return IsActive() && !_hasArrived && !IsAnticipating(now);
        }

        /// <summary>
        /// FSM: Is there a valid fallback path (for fallback/echo/squad logic)?
        /// </summary>
        public bool HasValidFallbackPath()
        {
            return _path.Count >= 2 && IsValid(_target);
        }

        /// <summary>
        /// FSM: Get fallback position, or fallback to default if none.
        /// </summary>
        public Vector3 GetFallbackPositionOrDefault(Vector3 defaultPos)
        {
            return HasValidFallbackPath() ? _target : defaultPos;
        }

        /// <summary>
        /// FSM: Should suppression/panic fallback trigger? (used for squad suppression, not anticipation)
        /// </summary>
        ...
        // --- END FSM-required APIs ---

        public void SetFallbackTarget(Vector3 pos, float now)
        {
            if (!IsValid(pos) || !IsBotMoveCapable(_bot))
                return;
        ...
        /// <summary>
        /// Main fallback tick: path-based, squad/personality safe, no teleport, no disables, never blocks FSM.
        /// Must be called only by BotBrain, never self-ticked.
        /// </summary>
        public void Tick(float deltaTime, float now, Action<CombatState, float> forceExit)
        {
            ...
            BotMovementHelper.SmoothMoveTo(_bot, adjusted, slow: true, cohesionScale: cohesion);
        ...
        private Vector3 FindBackupFallback(Vector3 origin)
        {
            ...
            return origin + Vector3.forward * 0.2f;
        }

        private static Vector3 ClampY(Vector3 v, Vector3 basePos)
        {
            if (Mathf.Abs(v.y - basePos.y) > 3f)
                v.y = basePos.y;
            return v;
        }
