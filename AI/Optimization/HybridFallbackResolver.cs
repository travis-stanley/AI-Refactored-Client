    ...
    public static class HybridFallbackResolver
    {
        ...
        public static Vector3? GetBestRetreatPoint(BotOwner bot, Vector3 threatDirection)
        {
            ...
            List<Vector3> navCoverPoints = NavPointRegistry.QueryNearby(
                origin,
                30f,
                pos => NavPointRegistry.IsCoverPoint(pos) &&
                       Vector3.Dot((pos - origin).normalized, -threatDirection.normalized) > 0.4f,
                coverOnly: true
            );

            if (navCoverPoints.Count > 0)
            {
                Vector3 best = navCoverPoints
                    .Select(pos => new
                    {
                        Position = pos,
            ...
            var fallbackHotspots = HotspotRegistry.QueryNearby(
                origin,
                40f,
                h => Vector3.Dot((h.Position - origin).normalized, -threatDirection.normalized) > 0.5f
            );

            if (fallbackHotspots.Count > 0)
            {
                return fallbackHotspots
                    .OrderBy(h => Vector3.Distance(origin, h.Position))
                    .First().Position;
            }
            ...
            if (TryLOSBlocker(origin, threatDirection, out Vector3 losPoint))
                return losPoint;
