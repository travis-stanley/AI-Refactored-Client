// <auto-generated>
//   AI-Refactored: AttackHandler.cs (Ultimate Realism – Beyond Diamond, EFT-Hardened, MoveHelper-Only Edition)
//   All world position/advance logic uses ONLY AIRefactored EFTPlayerUtil.GetPosition(BotOwner).
//   Bulletproof: No transform/position, no teleportation, no vanilla fallback, no disables.
//   MIT License.
// </auto-generated>
    ...
    public sealed class AttackHandler
    {
        ...
        public void Tick(float deltaTime)
        {
            try
            {
                ...
                // Get the bot's Player instance
                Player botPlayer = _bot != null ? _bot.GetPlayer : null;
                // Get the enemy's Player instance directly (enemy is already Player)
                Player enemyPlayer = enemy;

                if (botPlayer == null || enemyPlayer == null)
                    return;

                Vector3 botPos = EFTPlayerUtil.GetPosition(botPlayer);
                Vector3 targetPos = EFTPlayerUtil.GetPosition(enemyPlayer);
                ...
                // Human anticipation pause if the enemy moved a lot
                ...
                if (needsAdvance && now >= _anticipateUntil)
                {
                    ...
                    Vector3 advancePoint = GetEFTOnlyAdvancePoint(_bot, _cache, targetPos, distToTarget, out bool usedCover, botPos);
                    _lastAdvanceWasCover = usedCover;

                    float cohesion = Mathf.Clamp(_cache.PersonalityProfile?.Cohesion ?? 1f, 0.7f, 1.3f);

                    if (_bot.Mover != null && IsAdvancePointSafe(botPos, advancePoint))
                    {
                        ...
                        {
                            // Only issue move if enough delta from last issued
                            if ((_lastIssuedMove - advancePoint).sqrMagnitude > 0.03f || (now - _lastMoveTime) > 0.66f)
                            {
                                ...
                                Vector3 humanizedTarget = botPos + blended * Mathf.Clamp(moveDir.magnitude + overshoot, SafeMoveMin, SafeMoveMax);

                                if (!(_cache.Perception?.IsSuppressed ?? false))
                                    humanizedTarget = BotMovementHelper.ApplyMicroDrift(humanizedTarget, _bot.ProfileId, Time.frameCount, _cache.PersonalityProfile, _bot);

                                // Always NavMesh sample before move
                                if (NavMesh.SamplePosition(humanizedTarget, out NavMeshHit navHit, 0.65f, NavMesh.AllAreas))
                                {
                                    BotMovementHelper.SmoothMoveTo(_bot, navHit.position, false, cohesion);
                                    _lastIssuedMove = navHit.position;
                                    _lastMoveTime = now;
                                }
                            }
                        }
        ...
        /// </summary>
        private static Vector3 GetEFTOnlyAdvancePoint(BotOwner bot, BotComponentCache cache, Vector3 target, float dist, out bool usedCover, Vector3 botPos)
        {
            usedCover = false;
            ...
            if (cache.CoverPlanner != null && cache.CoverPlanner.TryGetBestCoverNear(target, botPos, out Vector3 cover))
            {
                usedCover = true;
            ...
            return BotNavHelper.TryGetSafeTarget(bot, out Vector3 fallback) ? fallback : botPos;
        }
        ...
        private static bool IsAdvancePointSafe(Vector3 current, Vector3 target)
        {
            ...
            if (!NavMesh.SamplePosition(target, out var navHit, 0.55f, NavMesh.AllAreas))
                return false;
